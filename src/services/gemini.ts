...
export const transcribeAudio = async (audioBase64: string) => {
  try {
    ...
    // Robustly determine mimeType from base64 header
    const matches = audioBase64.match(/^data:(.+);base64,(.+)$/);
    let mimeType = matches ? matches[1] : 'audio/webm';
    let base64Data = matches ? matches[2] : audioBase64;

    // Fallback: If no header found but comma exists (rare edge case)
    if (!matches && audioBase64.includes(',')) {
        const split = audioBase64.split(',');
        base64Data = split[1];
        // Attempt fallback detection if header was missing but data URL format suspected
